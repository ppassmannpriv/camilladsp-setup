...
function renderPipeline() {
    if (!state.config || !state.config.pipeline)
        return;
    ...
    state.config.pipeline.forEach((step, idx) => {
        ...
        if (step.names)
            details.innerText = `Filters: ${step.names.join(', ')}`;
        else if (step.channel !== undefined)
            details.innerText = `Channel: ${step.channel}`;
...
function switchView(viewName) {
    ...
    if (viewName === 'meters')
        ...
        el.pipelineView.classList.remove('hidden');
    else if (viewName === 'spectrum')
        el.spectrumView.classList.remove('hidden');
    else if (viewName === 'configs')
        el.configsView.classList.remove('hidden');
...
function createMeter(index, type) {
...
function renderMeters() {
    ...
    if (el.inputMeters.children.length === 0) {
        for (let i = 0; i < 8; i++)
            el.inputMeters.appendChild(createMeter(i, 'in'));
        for (let i = 0; i < 8; i++)
            el.outputMeters.appendChild(createMeter(i, 'out'));
    }
    // Update fill levels
